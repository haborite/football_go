...
- **かんたんルール説明**: 囲碁を知らない人向けの4枚のスライド（盤面イラスト付き）。初回訪問時に見るかどうかを案内し、タイトル画面からいつでも見返せる。最終スライドでは「埋めすぎると取られる」（二眼）のヒント付き
...
- **SNS共有**: 結果画面から対局結果（勝敗・目差・石数）と棋譜URLを X（Twitter）・Misskey・Mastodon のポスト画面に引き継いで共有。Misskeyは公式共有ページ（misskey-hub.net）経由、Mastodonは初回にサーバー名を入力（記憶される）
...
`public/ogp.png`（1200×630）をOGP / Twitterカードに使用（`index.html` のメタタグは
https://football-go.haborite.com/ を前提）。再生成は実際の3D盤面を撮影して合成する:

```sh
npm run ogp   # 要: npm run dev 起動中 + Edge
```
